...
int main(int argc, char* argv[]) {
  ...
  if (!kernel_data) {
    perror("malloc for kernel_data");
    fclose(f);
    return 1;
  }
  if (fread(kernel_data, 1, file_length, f) != (size_t)file_length) {
    perror("fread kernel_data");
    fclose(f);
    free(kernel_data);
    return 1;
  }
  ...
  if (cheese_create_kallsyms_lookup(&kallsyms_lookup, kernel_data,
                                    file_length)) {
    fprintf(stderr, "cheese_create_kallsyms_lookup failed\n");
    free(kernel_data);
    // The state of kallsyms_lookup is unknown, so it's unsafe to free sub-pointers
    return 1;
  }

  FILE* out_f = fopen("kallsyms.txt", "w");
  if (!out_f) {
    perror("fopen for kallsyms.txt");
    // Free allocated memory before exiting
    free(kernel_data);
    for (unsigned int i = 0; i < kallsyms_lookup.kallsyms_num_syms; i++) {
      free(kallsyms_lookup.decompressed_names[i]);
    }
    free(kallsyms_lookup.decompressed_names);
    return 1;
  }

  printf("writing kallsyms.txt...\n");
  for (unsigned int i = 0; i < kallsyms_lookup.kallsyms_num_syms; i++) {
    fprintf(out_f, "%016llx %c %s\n",
            kallsyms_lookup.kallsyms_relative_base +
                kallsyms_lookup.kallsyms_offsets[i],
            kallsyms_lookup.decompressed_names[i][0],
            kallsyms_lookup.decompressed_names[i] + 1);
  }
  fclose(out_f);
